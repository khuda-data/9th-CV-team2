...
Cafe 자동 관리 소프트웨어, "멈춰 외부 음식, 멈춰 장기간 점거"

CV 기반으로 카페 CCTV 영상을 분석해 장시간 좌석 점유와 자리비움 상태를 감지하고, 직원이 일관되게 응대할 수 있도록 돕는 운영 보조 시스템입니다.
...
최초 실행 전 `rois.json`을 생성해야 좌석이 표시됩니다.
...
- 영상 첫 프레임이 창으로 열립니다.
- 좌석마다 테이블 polygon을 클릭으로 지정한 뒤 `Enter`를 누릅니다.
- 같은 ID에 대응되는 좌석 polygon을 지정한 뒤 `Enter`를 누릅니다.
- 좌석 ID를 입력한 뒤 `Enter`를 누릅니다. 예: `A1`
- 모든 테이블/좌석 쌍을 지정한 뒤 `s`로 저장합니다.
- `Backspace`: 마지막 점 삭제, `r`: 마지막 ROI 삭제, `Esc`: 현재 입력 취소, `q`: 저장 없이 종료

### 2. 영상 소스 설정

`main.py`에서 영상 파일명 또는 카메라 인덱스를 지정합니다.
...
FastAPI 서버는 `http://127.0.0.1:8000`에서 시작됩니다.
...
Vite 개발 서버는 기본적으로 `http://127.0.0.1:5173`에서 시작되고, `/api`와 `/ws` 요청은 `http://localhost:8000`으로 프록시됩니다. 외부 백엔드를 붙일 때는 `VITE_API_BASE_URL`을 지정합니다.
...
1. `table_change.py`가 `tablePolygon`만 baseline 대비 구조 변화 면적으로 비교합니다.
2. `tableChangeScore`는 조명 보정 grayscale 차이에서 연결된 변화 영역을 강화한 구조 변화 점수입니다.
3. 구조 변화 점수가 `0.18` 이상이면 점유 변화로 진입하고, `0.10` 미만이면 해제합니다.
4. person 감지는 `personDetectionIntervalSeconds` 주기로 실행하고, person bbox의 hip anchor를 우선 본 뒤 pose keypoint의 팔꿈치/손목을 보조 앵커로 `seatPolygon` 안에 들어오는지 확인합니다.
5. 마지막 person 감지가 `personDetectionIntervalSeconds * 2.5` 이내면 테이블 변화와 무관하게 `SEATED`로 봅니다.
6. 최근 person 감지가 없고 tablePolygon 변화가 유지되면 `AWAY`로 전환하고 자리비움 타이머를 시작합니다.
7. 최근 person 감지도 없고 tablePolygon 변화도 baseline 수준으로 회복되면 `EMPTY`로 전환합니다.
...
알림 우선순위는 `OVERDUE > AWAY_TOO_LONG > NEAR_LIMIT > BELONGINGS_ONLY > NONE`입니다.

## 운영 정책 기본값

`PATCH /api/settings`로 런타임에 변경할 수 있습니다.
...
| `identityChangeDistance` | 0.3 | 임베딩 변화 후보 거리 기준 |
...
`setup_rois.py`가 생성하는 canonical schema는 다음과 같습니다. 좌표는 영상 해상도와 무관한 0~1 정규화 좌표입니다.
...
현재 schema는 `tablePolygon`과 `seatPolygon`을 분리해서 사용합니다.
...
- YOLO는 `person` class만 감지합니다.
- 컵, 노트북, 가방 같은 물체 class는 따로 분류하지 않습니다.
- 물건/짐 여부는 첫 프레임 baseline 대비 `tablePolygon` 변화로 표현하며 API에서는 `테이블 변화`로 노출합니다.
- 첫 프레임이 비어 있는 상태라는 전제가 중요합니다. 첫 프레임에 이미 물건이 있으면 baseline에 포함되어 이후 점유 변화로 보기 어렵습니다.
